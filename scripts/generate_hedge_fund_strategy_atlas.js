...
const OUT_NAME = "hedge-fund-strategy-atlas";
const IMG_DIR = path.join(ROOT, "assets", "images", OUT_NAME);
const OUT_DIR = path.join(ROOT, "outputs");
const NOTES_DIR = path.join(ROOT, "notes");
const COVER_SRC = "/Users/nuthdanai/.codex/generated_images/019e5acc-08a5-7e42-9613-5e1275e7745c/ig_0d632051e3ebab12016a132d308d14819190d70e56c52c4d90.png";
...
function base(slide) {
  ...
  <defs>
    <linearGradient id="bg" x1="0" x2="1" y1="0" y2="1">
      <stop stop-color="${C.bg}"/><stop offset="0.55" stop-color="${C.bg2}"/><stop offset="1" stop-color="#101010"/>
    </linearGradient>
    <radialGradient id="glow" cx="82%" cy="20%" r="65%">
      <stop stop-color="${C.green}" stop-opacity="0.18"/><stop offset="0.45" stop-color="${C.cyan}" stop-opacity="0.08"/><stop offset="1" stop-color="${C.bg}" stop-opacity="0"/>
    </radialGradient>
  </defs>
  ${rect(0, 0, W, H, "url(#bg)")}
  ${rect(0, 0, W, H, "url(#glow)")}
  <g opacity="0.45">${Array.from({ length: 20 }, (_, i) => line(120 + i * 90, 0, 120 + i * 90, H, "rgba(255,255,255,0.045)")).join("")}</g>
  <g opacity="0.45">${Array.from({ length: 10 }, (_, i) => line(0, 170 + i * 84, W, 170 + i * 84, "rgba(255,255,255,0.045)")).join("")}</g>
  ${rect(0, 0, W, 78, "rgba(18,18,18,0.72)")}
  ${rect(0, 1006, W, 74, "rgba(18,18,18,0.78)")}
  ${mono(`QUANTSERAS RESEARCH / ${n} / ${String(slides.length).padStart(2, "0")}`, 80, 52, 18, C.green)}
  ${mono("01 - QUANTSERAS DESIGN SYSTEM", 1485, 52, 16, C.low)}
  ${text(slide.title, 80, 142, 54, { fill: C.text, weight: 900, max: 38, lh: 1.02, family: "Arial Narrow, Arial Black, Arial, sans-serif" })}
  ${text(slide.subtitle, 84, 222, 25, { fill: C.muted, weight: 450, max: 84, lh: 1.25 })}
  ...
  ${mono("STRATEGY ATLAS", 1710, 1048, 16, C.low)}
`;
}
...
function panel(x, y, w, h, title, body, color = C.green) {
  return `<g>
    ${rect(x, y, w, h, C.panel, "rgba(255,255,255,0.10)", 1.2, 14)}
    ${rect(x, y, 7, h, color, "none", 0, 5)}
    ${text(title, x + 26, y + 48, 28, { fill: color, weight: 800, max: 28 })}
    ${text(body, x + 26, y + 96, 23, { fill: C.text, weight: 500, max: Math.floor(w / 13), lh: 1.18 })}
  </g>`;
}

function smallCard(x, y, w, h, title, body, color = C.green) {
  return `<g>
    ${rect(x, y, w, h, "rgba(29,29,29,0.88)", "rgba(255,255,255,0.10)", 1.2, 12)}
    ${mono(title, x + 24, y + 34, 18, color)}
    ${text(body, x + 24, y + 74, 21, { fill: C.text, weight: 520, max: Math.floor(w / 12), lh: 1.18 })}
  </g>`;
}
...
function slide05(slide) {
  ...
  const x0 = 315, y0 = 340, cw = 170, ch = 82;
  years.forEach((yr, i) => {
    s += rect(x0 + i * cw, y0 - 68, cw - 10, 48, "rgba(105,240,174,0.10)", "rgba(105,240,174,0.26)", 1, 8);
    s += mono(yr, x0 + i * cw + 80, y0 - 37, 19, C.green, 800, "middle");
  });
  performance.forEach((row, r) => {
    const y = y0 + r * ch;
    s += text(row.name, 94, y + 50, 23, { fill: C.text, weight: 800, max: 16 });
    row.vals.forEach((v, i) => {
      const x = x0 + i * cw;
      s += rect(x, y, cw - 10, ch - 12, heatColor(v), "rgba(255,255,255,0.12)", 1, 8);
      s += mono(v, x + (cw - 10) / 2, y + 45, 23, C.text, 800, "middle");
    });
    s += text(row.take, 1190, y + 31, 19, { fill: C.muted, weight: 500, max: 47, lh: 1.12 });
  });
  s += `${text("Public-source caveat: selected cells use secondary HFR reporting or qualitative public-note language where a release does not state a full-year total figure.", 94, 930, 17, { fill: C.low, max: 102 })}
        ${pill("GREEN = STRONG", 1215, 855, C.green, 195)}
        ${pill("RED = STRESS", 1430, 855, C.red, 180)}
        ${pill("AMBER = HIGH VOL", 1630, 855, C.amber, 190)}`;
  return s + end();
}
...
function slide17(slide) {
  ...
  s += text("Full inventory and manager examples are written in the research note.", 90, 1000, 20, { fill: C.muted, weight: 600, max: 80 });
  return s + end();
}
